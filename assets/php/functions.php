...
function createUser($data){
 ...
 $query = "INSERT INTO users(first_name,last_name,gender,email,username,password,ac_status,uidUsers) ";
 $query.="VALUES ('$first_name','$last_name',$gender,'$email','$username','$password',1,'$email')";
 mysqli_query($db,$query); 
 /*Get the id of the last user who's signed up!*/
  $req="SELECT id FROM users WHERE uidUsers='$email'";
...
$req2="INSERT INTO imgupload(userid,status)";
$req2.= "VALUES('$id',1)";

    
 return mysqli_query($db,$req2);
}
...
$user_id = $_SESSION['userdata']['id'];
    ...
    //     var_dump($image_name, $image);
    //     move_uploaded_file($image['tmp_name'],"$image_dir/$image_name");
    // die();
