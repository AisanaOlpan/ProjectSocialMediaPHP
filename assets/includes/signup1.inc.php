<?php
  		if ( ! (empty($_POST['uid'])|| trim($_POST['uid'])=='') && !(empty($_POST['mail'])|| trim($_POST['mail'])=='') && !(empty($_POST['pwd'])|| trim($_POST['pwd'])=='') && !(empty($_POST['pwd-repeat'])|| trim($_POST['pwd-repeat'])=='') ) {
  			...
  			if(!filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/",$_POST['uid'])){
				...
				try{
					$db=new PDO('mysql:host=localhost;dbname=pictogram;charset=utf8','root','',array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
				}
				catch(Exception $err){
				...
				if ($i!==0){
					...
					$requete = $db->prepare('INSERT INTO users (uidUsers,email,password,first_name,last_name,gender,username,ac_status) VALUES(:u,:e,:p,"Ais","Olp",2,"Ais",1)');
					$requete->execute(array('u' => $_POST['uid'],'e' => $_POST['mail'],'p' =>$hashedPwd ));
					$requete->closeCursor();
          /*Get the id of the last user who's signed up!*/
          $req=$db->prepare('SELECT id FROM users WHERE uidUsers=:u');
  				$req->execute(array('u'=>$_POST['uid']));
          $enreg=$req->fetch() ;
  				$id=$enreg['id'];
